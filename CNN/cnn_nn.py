...
# ----------------- Helpers -----------------
def build_image_lookup(images_dir=IMAGES_DIR):
    """Build a lookup dictionary mapping image IDs to full file paths."""
    print(f"Building image lookup from {images_dir}...")
    ...
    # Get all image files (both .jpg and .JPG) and handle duplicates
    jpg_files = list(images_dir.glob("*.jpg"))
    jpg_upper_files = list(images_dir.glob("*.JPG"))

    print(f"Found {len(jpg_files)} .jpg files")
    print(f"Found {len(jpg_upper_files)} .JPG files")

    # Combine and remove duplicates based on stem (filename without extension)
    all_files = jpg_files + jpg_upper_files
    unique_files = {}

    for img_path in all_files:
        stem = img_path.stem.lower()  # Use lowercase stem as key
        if stem not in unique_files:
            unique_files[stem] = img_path
        # If duplicate found, keep the first one (or you could prefer .jpg over .JPG)

    image_files = list(unique_files.values())
    print(f"Unique image files after deduplication: {len(image_files)}")

    # Show some sample filenames for debugging
    if image_files:
        print("Sample filenames:", [f.name for f in image_files[:10]])

    for img_path in image_files:
        # Extract ID from filename (remove extension)
        filename_id = img_path.stem  # e.g., "0000", "0001", "7837"

        try:
            # Convert filename to integer to get the actual ID
            numeric_id = int(filename_id)

            # Only store if not already present (avoid overwriting)
            if numeric_id not in image_lookup:
                image_lookup[numeric_id] = str(img_path)
                image_lookup[str(numeric_id)] = str(img_path)

        except ValueError:
            # If somehow not numeric, skip this file
            print(f"Warning: Non-numeric filename found: {filename_id}")
            continue

    # Count unique numeric IDs
    unique_numeric_ids = len([k for k in image_lookup.keys() if isinstance(k, int)])
    print(f"Built lookup with {unique_numeric_ids} unique image IDs")

    # Show some sample lookup keys for debugging
    sample_keys = sorted([k for k in image_lookup.keys() if isinstance(k, int)])[:10]
    print("Sample numeric lookup keys:", sample_keys)

    return image_lookup


def find_image_path(img_id, image_lookup):
    """Find image path using the prebuilt lookup dictionary."""

    # First, try to convert the CSV ID to integer and look it up directly
    try:
        int_id = int(img_id)
        if int_id in image_lookup:
            return image_lookup[int_id]
    except (ValueError, TypeError):
        pass

    # Try the ID as string
    str_id = str(img_id).strip()
    if str_id in image_lookup:
        return image_lookup[str_id]
...
def test_image_lookup(image_lookup, test_ids=[7837, 5257, 7336, 3263]):
    """Test the image lookup with sample IDs from your CSV."""
    print("\nTesting image lookup:")
    for test_id in test_ids:
        path = find_image_path(test_id, image_lookup)
        if path:
            print(f"ID {test_id}: Found -> {path}")
        else:
...
print(f"Looking up {len(train_df)} training images...")

# Debug: show sample IDs from CSV
sample_csv_ids = train_df[train_id_col].head(10).tolist()
print(f"Sample CSV IDs: {sample_csv_ids}")
print(f"CSV ID types: {[type(x).__name__ for x in sample_csv_ids]}")

train_df['filepath'] = train_df[train_id_col].apply(lambda x: find_image_path(x, image_lookup))
missing_train = train_df['filepath'].isna().sum()
found_train = train_df['filepath'].notna().sum()

print(f"Found: {found_train} train images")
print(f"Missing: {missing_train} train images")

if missing_train > 0:
    print("Sample missing IDs:", train_df[train_df['filepath'].isna()][train_id_col].head(10).tolist())

    # Debug: try to find what's different
    missing_ids = train_df[train_df['filepath'].isna()][train_id_col].head(5).tolist()
    print("Debugging missing IDs:")
    for mid in missing_ids:
        print(f"  ID {mid} (type: {type(mid).__name__}):")
        print(f"    - Checking as string '{str(mid)}': {str(mid) in image_lookup}")
        print(f"    - Checking as int {mid}: {mid in image_lookup}")
        # Check if there's a similar key
        similar_keys = [k for k in list(image_lookup.keys())[:100] if str(mid) in str(k) or str(k) in str(mid)]
        if similar_keys:
            print(f"    - Similar keys found: {similar_keys[:5]}")

if found_train > 0:
    print("Sample found IDs:", train_df[train_df['filepath'].notna()][train_id_col].head(5).tolist())

train_df = train_df.dropna(subset=['filepath']).reset_index(drop=True)

print(f"Looking up {len(test_df)} test images...")
test_df['filepath'] = test_df[test_id_col].apply(lambda x: find_image_path(x, image_lookup))
missing_test = test_df['filepath'].isna().sum()
if missing_test > 0:
    print(f"Warning: {missing_test} test images not found in {IMAGES_DIR}.")
    print("Sample missing test IDs:", test_df[test_df['filepath'].isna()][test_id_col].head().tolist())
...
cw = class_weight.compute_class_weight(class_weight='balanced', classes=np.arange(num_classes),
